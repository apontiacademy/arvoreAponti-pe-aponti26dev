...
Phase 2 — Drag-and-drop editor (2026-07-17): done — add/edit/delete/reorder link blocks on `PageEditPage`, using Framer Motion's `Reorder` (not a dedicated DnD library, see "Architecture"). Per-block validation is minimal — see "Known gaps".
...
React Hook Form + Zod (+ `@hookform/resolvers`) are installed and wired (Login, Page create/edit forms — **not** the link-block editor, which uses plain controlled inputs, see "Architecture"). Sonner is installed and wired — `<Toaster />` renders once in `src/app/providers.tsx`, `toast.success`/`toast.error` used for mutation feedback (see "Screens" below). Framer Motion is installed — only `Reorder`/`useDragControls` are used so far (drag-and-drop reordering of link blocks), not its animation/gesture APIs more broadly. Lucide React (`lucide-react`) is installed and is shadcn's configured icon library — **this version ships no brand/social icons** (no `Instagram`, `Youtube`, `Facebook`, etc.); social block types use generic icons instead (see "Architecture").
...
Login, Dashboard, Page list (search + duplicate), Create/Edit page (drag-and-drop editor), Public page view, Analytics, Profile, Settings, 404. Route placeholders for all of these exist under `src/routes/`. `Login`, `Dashboard`, Page CRUD, and the link-block editor now have real behavior (below); Profile, Settings, and the public page view still render static placeholder text pending their build-order slot.
...
1. Public page (rendering `PublicPagePage` for real + view/click analytics capture)
2. Analytics dashboard
3. Tests and refinement
...
- **`useDuplicatePage` only copies the page row**, not its `links`/`social_links` — now that links exist as a feature (see "Screens / feature surface"), this should be fixed rather than deferred again.
- **Link blocks have no validation.** `LinkBlockCard` autosaves whatever is typed — an empty `url` on a type that needs one (everything except `title`/`text`), a non-email string on `email`, etc. all save silently. There's no Zod schema for block fields (unlike `pageFormSchema`) and no per-type required-field enforcement. This was scoped out for time, not a deliberate design call — worth fixing before this is used for real content.
